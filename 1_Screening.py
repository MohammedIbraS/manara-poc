...
def construct_pico_query(
    ...
    terms = []
    if p:
        terms.append(f'"{p}"[Title/Abstract]')
    if i:
        terms.append(f'"{i}"[Title/Abstract]')
    if c:
        terms.append(f'"{c}"[Title/Abstract]')
    if o:
        terms.append(f'"{o}"[Title/Abstract]')
...
def get_abstract(article_id):
    ...
    try:
        handle = Entrez.efetch(
            db="pubmed", id=article_id, rettype="abstract", retmode="xml"
        )
...
def search_pubmed(query, pico_query_text, max_results=10):
    ...
    try:
        handle = Entrez.esearch(
            db="pubmed", term=query, retmax=str(max_results), sort="relevance"
        )
        record = Entrez.read(handle)
        handle.close()
        ...
        if not id_list:
            ...
            st.session_state.error = (
                "No articles found with that specific combination of filters."
            )
            return
        ...
        if abstracts_to_score:
            ...
            abstract_embeddings = model.encode(
                abstracts_to_score, convert_to_tensor=True
            )
...
def highlight_text(text, keywords):
    ...
    pattern = re.compile(
        r"\b(" + "|".join(re.escape(kw) for kw in keyword_list) + r")\b", re.IGNORECASE
    )
...
if st.session_state.error:
    ...
    st.info(
        "Enter your search criteria in the sidebar and click 'Search PubMed' to begin."
    )
else:
    for article in st.session_state.search_results:
        ...
        with st.expander(f"*{article['status'].upper()}*: {display_title}"):
            ...
            highlighted_abstract = highlight_text(
                abstract, st.session_state.pico_query_text
            )
            ...
            with col2:
                st.button(
                    "🤔 Maybe",
                    key=f"maybe_{article['id']}",
                    on_click=set_status,
                    args=(article["id"], "maybe"),
                    use_container_width=True,
                )
            ...
            with col3:
                st.button(
                    "❌ Exclude",
                    key=f"exclude_{article['id']}",
                    on_click=set_status,
                    args=(article["id"], "excluded"),
                    use_container_width=True,
                )
